...
void compute_aabb_seq(vector<Node>& leaf_nodes, vector<Node>& in_nodes,
	...
	if(r_child != INVALID_U32) {
		if(in_nodes[idx].r_is_leaf) {
			...
			in_nodes[r_child].min_x = mins[0];
			in_nodes[r_child].min_y = mins[1];
			in_nodes[r_child].min_z = mins[2];

			in_nodes[r_child].max_x = maxs[0];
			in_nodes[r_child].max_y = maxs[1];
			in_nodes[r_child].max_z = maxs[2];
		}
